...
Work in progress versions typically starts at XYY99 then bounce up to XYY00, XYY01 etc. when release tagging happens) */
        ...
        public const float WINDOWS_HOVER_PADDING = 4f;

        /** Reduce visual noise by only highlighting the border after a certain time. */
        public const float WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER = 0.04f;

        /** Lock scrolled window (so it doesn't pick child windows that are scrolling through) for a certain time, unless mouse moved. */
        public const float WINDOWS_MOUSE_WHEEL_SCROLL_LOCK_TIMER = 0.7f

        // Tooltip offset
        public Vector2 TOOLTIP_DEFAULT_OFFSET = new Vector2(16, 10);            // Multiplied by g.Style.MouseCursorScale

        // Test engine hooks (imgui-test)
        public bool IMGUI_ENABLE_TEST_ENGINE = true;

        // @JvmField
        public bool IMGUI_DEBUG_TOOL_ITEM_PICKER_EX = false;

        public bool IMGUI_DISABLE_DEBUG_TOOLS = false;
        ...
        public const int UNICODE_CODEPOINT_MAX = /*sizeof(ImWchar) == 2 ?*/ 0xFFFF; /*: 0x10FFFF*/

        /** Standard invalid Unicode code point.
         *
         *  Invalid Unicode code point (standard value). */
        public const int UNICODE_CODEPOINT_INVALID = 0xFFFD;

        // @JvmField
        public bool MINECRAFT_BEHAVIORS = false;
